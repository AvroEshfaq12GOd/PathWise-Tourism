...
function estimateFromWeather(base: number, temp?: number, humidity?: number) {
  let score = base;
  if (temp !== undefined && temp > 30) score += 4;
  if (humidity !== undefined && humidity > 75) score += 3;
  return score;
}

export async function computeLiveDensity(site: SiteDoc) {
  ...
  let source: 'besttime' | 'weather' | 'system' = 'system';
