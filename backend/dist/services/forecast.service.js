...
function buildForecastPoints(base, trend, horizonHours = 4) {
    const points = [];
    let last = base;
...
function buildForecastPointsFromSeries(series, horizonHours = 4) {
    const points = [];
    const now = new Date();
...
export async function generateSiteForecast(site, horizonHours = 4) {
...
export async function getLatestForecast(siteId) {
    return ForecastModel.findOne({ siteId }).sort({ generatedAt: -1 }).lean();
}
export async function refreshSiteForecast(siteId) {
