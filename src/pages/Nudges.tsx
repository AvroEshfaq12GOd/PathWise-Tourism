    ...
    <motion.div
      ...
      <div className="px-5 mt-2">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto hide-scrollbar">
          {['All', 'Routes', 'Locations', 'Off-peak'].map((filter, i) =>
          <button
            key={filter}
            className={`px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap ${i === 0 ? 'bg-slate-800 text-white' : 'bg-white text-slate-600 border border-slate-200'}`}>
                ...
                <motion.div
                  ...
                  className="bg-white rounded-2xl p-5 shadow-soft border border-slate-100 relative overflow-hidden">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-600">
                          <span className="flex items-center gap-1">
                            <Navigation size={12} /> {nudge.travelTimeMin} min
                          </span>
                          <span>•</span>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-brand-700 font-bold text-sm bg-brand-50 px-3 py-1.5 rounded-lg">
                        <Gift size={16} />
                        {nudge.incentive}
                      </div>
                      <button
                      onClick={() => handleAccept(nudge.id)}
                      className="bg-slate-900 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-sm">
